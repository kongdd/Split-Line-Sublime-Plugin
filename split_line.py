    ...
    def run(self, edit, **args):
        ...
        def split_text(selected_text, indent_size):
            ...
            if "\n" not in selected_text:
                if ((',' in selected_text.strip(' ')
                    ...
                    # print('pre_multi', pre_multi, 'tarray', tarray, 'post_multi', post_multi)
                    ...
                    for i, l in enumerate(tarray):
                        if (
                            ...
                            multi_line_text += indent + sep + l + ',\n'
                        else:
                            multi_line_text += indent + sep + l + '\n'

                    multi_line_text += indent + post_multi

                    print('multi_line_text', multi_line_text)
        ...
        for region in self.view.sel():
            if region.empty():
                ...
                indent_text = selected_text[:len(
                    selected_text) - len(selected_text.lstrip(' ').lstrip('\t').lstrip(' '))]
                indent_size = 0
